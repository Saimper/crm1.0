...
final class AdminCategoriasTicket extends AbstractAdminCatalogo
{
    ...
    {
        $items = DB::table('categorias_ticket')
            ->where('proyecto_id', $this->proyectoActivoId())
            ->orderBy('orden')->orderBy('codigo')
            ->get();

        return view('cx::livewire.admin-categorias-ticket', ['items' => $items]);
    }
}
