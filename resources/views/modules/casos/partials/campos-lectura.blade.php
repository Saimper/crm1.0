...
<div class="workspace-fields" wire:key="campos-lectura-{{ $casoActivo->id }}"
    ...
    <div class="workspace-fields-toolbar">
        <label class="workspace-fields-search">
            ...
            <input type="search" x-model.debounce.150ms="busqueda" placeholder="Buscar un dato…" aria-label="Buscar en los datos de la cuenta" />
        </label>
        <label class="workspace-fields-toggle">
            <input type="checkbox" x-model="mostrarVacios" /> Mostrar vacíos
        </label>
    </div>
    <p class="workspace-fields-count" role="status" x-text="visibles.length + ' de ' + datos.length + ' campos'"></p>
    ...
    @foreach($gruposCamposCaso as $grupo)
        ...
        <details open class="workspace-field-group" x-show="@js($idsGrupo).some(id => visibles.includes(id))">
            <summary>
                {{ $grupo['nombre'] === __('casos.fields_ungrouped') ? 'Datos de la cuenta' : $grupo['nombre'] }}
            </summary>
    ...
    <div x-show="visibles.length === 0" x-cloak class="workspace-fields-empty">
        <p x-text="busqueda.trim() ? 'No hay datos que coincidan con la búsqueda.' : 'Esta cuenta no tiene campos con información.'"></p>
        <button type="button" class="btn-link" x-on:click="busqueda = ''; mostrarVacios = true">Ver todos los campos</button>
    </div>
</div>
