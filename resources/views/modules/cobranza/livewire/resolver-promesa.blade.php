...
<div class="inline-flex flex-wrap items-center gap-1.5">
    <button type="button" wire:click="abrir('cumplida')" class="btn btn-sm btn-success">{{ __('cobranza.btn_cumplida') }}</button>
    <button type="button" wire:click="abrir('rota')" class="btn btn-sm btn-secondary text-danger-600 border-danger-200 hover:bg-danger-50">{{ __('cobranza.btn_rota') }}</button>
    <button type="button" wire:click="abrir('cancelada')" class="btn btn-sm btn-ghost text-ink-500">{{ __('cobranza.btn_cancelar') }}</button>
    ...
    @if($modalAbierto)
        <div class="scrim" wire:click="cerrar" wire:key="scrim-resolver-{{ $compromisoId }}"></div>
        <div class="modal max-w-[400px] p-5" role="dialog" aria-modal="true" wire:key="modal-resolver-{{ $compromisoId }}">
            <div class="text-md font-semibold text-ink mb-3">
                {{ __('cobranza.modal_title') }} <span class="capitalize">{{ $accion }}</span>
            </div>
            <label class="field-label" for="fecha-resolver-{{ $compromisoId }}">{{ __('cobranza.fecha_resolucion') }}</label>
            <input type="date" id="fecha-resolver-{{ $compromisoId }}" wire:model="fechaResolucion" class="input font-mono"/>
