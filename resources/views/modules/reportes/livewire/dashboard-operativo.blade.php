...
<div class="space-y-4">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        ...
        <div class="rounded-lg border border-ink-200 bg-white p-4">
            <div class="text-[10px] uppercase tracking-wider text-ink-500">Efectividad</div>
            <div class="text-2xl font-semibold text-brand-700 mt-1">{{ number_format($efectividad, 1) }}%</div>
        </div>
        <div class="rounded-lg border border-ink-200 bg-white p-4">
            <div class="text-[10px] uppercase tracking-wider text-ink-500">Total gestiones</div>
        ...
        <div class="rounded-lg border border-success-200 bg-success-50 p-4">
            <div class="text-[10px] uppercase tracking-wider text-success-700">Compromisos vigentes</div>
        ...
        <div class="rounded-lg border border-danger-200 bg-danger-50 p-4">
            <div class="text-[10px] uppercase tracking-wider text-danger-700">Compromisos vencidos</div>
    ...
    <section class="rounded-lg border border-ink-200 bg-white overflow-hidden">
        ...
        @if($ranking->isEmpty())
            <div class="p-6 text-sm text-ink-500 text-center">Sin actividad en el rango seleccionado.</div>
        @else
    ...
    <section class="rounded-lg border border-ink-200 bg-white overflow-hidden">
        ...
        @if($gestiones->isEmpty())
            <div class="p-6 text-sm text-ink-500 text-center">Sin gestiones registradas.</div>
        @else
            <table class="min-w-full divide-y divide-ink-200 text-sm">
                <thead class="bg-ink-50 text-xs uppercase tracking-wider text-ink-600">
                    <tr>
                        ...
                        <tr>
                            ...
                            <td class="px-3 py-2 text-xs">
                                ...
                                @if($g->es_contacto_efectivo)
                                    <span class="inline-block rounded px-1.5 py-0.5 text-[10px] bg-success-50 text-success-800">efectivo</span>
                                @endif
                            </td>
