<div class="space-y-6" @if($progreso !== null && $progreso->enCurso() && $progreso->estado->value === 'procesando') wire:poll.2s @endif>
    ...
    <section class="rounded-lg border border-ink-200 bg-white p-6 space-y-4">
        <div>
            <h3 class="text-sm font-semibold uppercase tracking-wider text-ink-700">Importar {{ $tipoOperacion }}</h3>
            <p class="text-xs text-ink-500 mt-1">
                Tipo de operación del proyecto: <code>{{ $tipoOperacion }}</code>. Solo casos de este tipo.
            </p>
            <p class="text-xs text-ink-500 mt-1">
                <strong>Columnas obligatorias:</strong> <code>{{ implode(', ', $columnasEsperadas) }}</code>
            </p>
        </div>
            ...
            <form wire:submit.prevent="guardarArchivo" class="space-y-3">
                ...
                <div>
                    ...
                    <select wire:model="modo" class="mt-1 block w-full text-sm border-ink-300 rounded-md">
                        <option value="merge">merge — rellena solo campos vacíos en casos existentes</option>
                        <option value="skip_duplicados">skip_duplicados — ignora casos existentes (continúa)</option>
                        <option value="overwrite">overwrite — pisa todos los campos en casos existentes</option>
                    </select>
                </div>
    ...
    <section class="rounded-lg border border-ink-200 bg-white overflow-hidden">
        ...
        @if($historial->isEmpty())
            <div class="p-6 text-sm text-ink-500 text-center">Aún no hay importaciones de casos en este proyecto.</div>
        @else
