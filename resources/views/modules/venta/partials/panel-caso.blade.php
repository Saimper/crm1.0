...
@if($lead)
    <div class="card card-pad" style="space-y:12px;">
        <div class="flex items-start justify-between gap-4">
            <div>
                <div class="text-[10px] uppercase tracking-wider" style="color:var(--text-tertiary);">{{ __('venta.panel_label') }}</div>
                <div class="text-lg font-semibold font-mono" style="color:var(--text);">{{ $lead->codigo_lead }}</div>
            </div>
            <div class="text-right">
                <div class="text-xs uppercase tracking-wider font-semibold" style="color:var(--text-secondary);">{{ __('venta.valor_estimado') }}</div>
        ...
        <dl class="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs" style="margin-top:12px;">
            <div>
                <dt style="color:var(--text-tertiary);">{{ __('venta.producto') }}</dt>
                <dd class="font-medium" style="color:var(--text);">{{ $lead->producto_nombre ?? '—' }}</dd>
            </div>
            <div>
                <dt style="color:var(--text-tertiary);">{{ __('venta.etapa_embudo') }}</dt>
            ...
            <div>
                <dt style="color:var(--text-tertiary);">{{ __('venta.origen') }}</dt>
                <dd class="font-medium" style="color:var(--text);">{{ $lead->origen_lead ?? '—' }}</dd>
            </div>
            <div>
                <dt style="color:var(--text-tertiary);">{{ __('venta.primer_contacto') }}</dt>
                <dd class="font-medium" style="color:var(--text);">
            ...
            @if($lead->fecha_estimada_cierre)
                <div class="col-span-2 sm:col-span-4">
                    <dt style="color:var(--text-tertiary);">{{ __('venta.cierre_estimado') }}</dt>
