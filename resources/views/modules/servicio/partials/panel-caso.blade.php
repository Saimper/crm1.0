...
@if($servicio)
    <div class="rounded-md border border-brand-200 bg-brand-50 p-4 space-y-3">
        <div class="flex items-start justify-between gap-4">
            <div>
                <div class="text-[10px] uppercase tracking-wider text-brand-700">{{ __('servicio.panel_label') }}</div>
        ...
        <dl class="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
            <div class="col-span-2 sm:col-span-4">
                <dt class="text-brand-700">{{ __('servicio.direccion') }}</dt>
                <dd class="font-medium text-brand-900">{{ $servicio->direccion_servicio ?? '—' }}</dd>
            </div>
            <div>
                <dt class="text-brand-700">{{ __('servicio.tecnico_asignado') }}</dt>
