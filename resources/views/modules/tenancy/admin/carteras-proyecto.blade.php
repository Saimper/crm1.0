<div class="page">
    <div class="page-header">
        <div>
            <h1 class="page-title">Carteras del proyecto</h1>
            <div class="page-subtitle">Subdivisiones operativas — agrupan casos del proyecto</div>
        </div>
    ...
    @if(session('admin-carteras-ok'))
        <div class="alert alert-success" style="margin-bottom:14px;">{{ session('admin-carteras-ok') }}</div>
    @endif
    ...
    <div class="card" style="padding:0;">
        <div style="padding:12px 16px;border-bottom:1px solid var(--border);display:flex;gap:10px;align-items:center;">
            ...
            <span style="font-size:12px;color:var(--text-tertiary);">{{ $carteras->count() }} registros</span>
        </div>
        ...
        @if($carteras->isEmpty())
            <div class="empty">
                ...
                <div class="empty-desc">Crea la primera cartera para empezar a clasificar casos.</div>
            </div>
        @else
            <table class="table table-compact table-clickable">
                ...
                <tbody>
                    @foreach($carteras as $c)
                        <tr wire:key="cartera-{{ $c->id }}" wire:click="abrirFormEditar({{ $c->id }})">
    ...
    @if($formVisible)
        <div class="scrim" wire:click="cerrarForm" wire:key="form-cartera-scrim"></div>
        <div class="drawer" wire:key="form-cartera">
            ...
            <div class="drawer-body">
                <div style="display:grid;grid-template-columns:1fr;gap:14px;">
                    ...
                    <div>
                        ...
                        <textarea wire:model="form.descripcion" rows="3"
            ...
            <div class="drawer-footer">
                @if($editandoId !== null)
                    @php
                        $row = \App\Modules\Tenancy\Infrastructure\Persistence\Models\CarteraModel::query()->find($editandoId);
                    @endphp
                    @if($row && $row->activo)
                        <button type="button" wire:click="desactivar({{ $editandoId }})"
                                wire:confirm="¿Desactivar esta cartera? Los casos existentes no se afectan."
