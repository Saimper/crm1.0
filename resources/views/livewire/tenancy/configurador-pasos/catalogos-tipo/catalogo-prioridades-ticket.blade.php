<div>
    ...
    <div class="card" style="padding:0;">
        <div style="padding:12px 16px;border-bottom:1px solid var(--border);display:flex;gap:10px;align-items:center;">
            ...
            <span style="flex:1;"></span><span style="font-size:12px;color:var(--text-tertiary);">{{ __('configurador.prioridades_ticket.n_prioridades', ['n' => $rows->count()]) }}</span>
            <button type="button" wire:click="abrirFormCrear" class="btn btn-primary"><x-ui.icon name="plus" :size="14"/><span>{{ __('configurador.prioridades_ticket.nueva') }}</span></button>
        </div>
        @if($rows->isEmpty())
            <div class="empty"><div class="empty-icon"><x-ui.icon name="folder" :size="32"/></div><div class="empty-title">{{ __('configurador.prioridades_ticket.sin_titulo') }}</div></div>
        @else
            <table class="table table-compact table-clickable">
                <thead><tr><th style="width:160px;">{{ __('configurador.campo_codigo') }}</th><th>{{ __('common.name') }}</th><th class="num" style="width:80px;">{{ __('configurador.prioridades_ticket.col_peso') }}</th><th class="num" style="width:70px;">{{ __('configurador.campo_orden') }}</th><th style="width:110px;">{{ __('configurador.campo_estado') }}</th><th style="width:60px;"></th></tr></thead>
                <tbody>
                @foreach($rows as $r)
    ...
    @if($formVisible)
        ...
        <div class="drawer" wire:key="cat-pt-drawer">
            <div class="drawer-header"><div style="font-size:14px;font-weight:600;">{{ $editandoId === null ? __('configurador.prioridades_ticket.drawer_nueva') : __('configurador.prioridades_ticket.drawer_editar') }}</div>
            ...
            <div class="drawer-body"><div style="display:grid;grid-template-columns:1fr;gap:14px;">
                ...
                <div><label class="field-label">{{ __('configurador.prioridades_ticket.campo_peso') }}</label><input type="number" min="0" wire:model="form.peso" class="input @error('form.peso') input-error @enderror"/>@error('form.peso')<div class="field-error">{{ $message }}</div>@enderror</div>
            ...
            <div class="drawer-footer">
                @if($editandoId !== null)<button type="button" wire:click="eliminar({{ $editandoId }})" wire:confirm="{{ __('configurador.prioridades_ticket.confirm_eliminar') }}" class="btn btn-ghost" style="color:var(--danger-text);margin-right:auto;">{{ __('common.delete') }}</button>@endif
