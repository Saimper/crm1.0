...
| Screen-pop ViciDial → CRM (fase 1) — el wrapper abre la ficha sola al conectar la llamada (PR wrapper #1); el CRM aterriza con aviso y alta prellenada cuando no hay ficha, no abre fichas ambiguas y ancla el writeback a la persona de la llamada (PR #18). 0 migraciones. 24 tests nuevos. | ✅ 2026-09-09 |
...
**Screen-pop desde ViciDial (fase 1, 2026-09-09):** el wrapper abre el CRM con un handshake por llamada en cuanto conoce el lead (precarga en RINGING/DIALING) y cambia a la pestaña CRM al pasar a `IN_CALL`; la ficha se retiene tras la disposición. Contrato sin claims nuevos. Lado CRM:
- `PayloadJwt` recorta `identificacion`, `tipo_identificacion_codigo` y `numero_prestamo`; en blanco = ausente.
- `ConsumirJwtHandshake` resuelve en orden `numero_prestamo` → `identificacion` (+tipo si viene). Más de una persona con la misma identificación (dos tipos) = **ambigua**: no se abre ninguna. Output lleva `identificacionNoResuelta`, `tipoIdentificacionCodigo`, `identificacionAmbigua`.
- Aterrizaje sin ficha: `/proyectos/{p}/bandeja?sin_persona=…&tipo=…[&ambigua=1]`. `Bandeja` (mount, `#[Locked]`) pinta el aviso y, si `personas.crear`, enlaza a `personas/crear?identificacion=…&tipo=…`; `CrearPersona::mount` prellena desde la query.
- Sesión: `crm_persona_public_id` = persona que abrió el handshake. `EmiteWritebackFicha::emitirWritebackFicha($changes, $personaPublicId)` omite el writeback si el gestor edita otra ficha; sin persona anclada se emite como antes. Los 4 Livewire emisores pasan el `public_id`.
- Limiter nombrado `sso-handshake` (`IntegracionServiceProvider`): 120/min por usuario con sesión, 60/min por IP en el primer handshake.
...
- **F34B**: cierre 2026-04-30. Resueltos 11/11 P0 (1 parcial: tests multi-tenancy 6 de 20 módulos cubiertos) + 19/19 P1. Pendientes a F34C: 14 módulos sin test multi-tenancy explícito, ImportarCasos→registro link (Personas ya cubierto), los 13 P2 y 3 P3 originales.
